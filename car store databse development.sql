...
`Order Year` int default Null,
`Order Value` decimal(10,2)
);
create fulltext index product on sales
(`product line`);
...
create table `product lines`(
`product line` varchar(50),
`Text Description` varchar(5000),
`Html Description` mediumtext,
image mediumblob,
primary key(`Product Line`)
);
insert into `Product Lines`
select * from classicmodels.productlines;

create table offices (
`Office Code` varchar(50) not null,
City varchar(50),
Phone varchar(50),
...
Email varchar(50),
`Office Code` varchar(50),
`Report To` varchar(50),
`Job Title` varchar(50),
primary key ( `Employee Number`),
foreign key(`office code`) references offices(`office code`) on update cascade on delete cascade
);
...
`Product Code` varchar(50) not null,
`Product Name` varchar(70),
`Product Line` varchar(50),
`Product Scale` varchar(50),
`Product Vendor` varchar(50),
...
foreign key(`Product Line`) references `product lines`(`Product Line`) on update cascade on delete cascade
);
...
`Product Code` varchar(70),
`Quantity Ordered` int,
`Price` Decimal(10,2),
`Orderline Number` int,
Primary key(`Order Number`),
foreign key(`Product Code`) references Products(`Product Code`) on update cascade on delete cascade
);

insert ignore into `order details`
...
`customer Number` int,
primary key (`Order Number`),
foreign key(`Order Number`) references `order details`(`Order Number`) on update cascade on delete cascade
);
insert into orders
...
`Postal Code` varchar(50),
Country varchar(50),
`State Rep Employee Number` int,
`Credit Limit` decimal(10,2),
primary key (`Customer Number`),
 foreign key(`State Rep Employee Number`) references employees(`employee number`) on update cascade on delete cascade
);
